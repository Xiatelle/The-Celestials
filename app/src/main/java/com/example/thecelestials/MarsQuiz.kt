...
class MarsQuiz : AppCompatActivity() {
    ...
    private val correct = "OF ALL THE\nIRON IN THE SOIL"
    private val incorrect = "IT HAS TWO\nMOONS"

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_mars_quiz)

        choiceAImage = findViewById(R.id.choiceAImage)
        choiceBImage = findViewById(R.id.choiceBImage)
        marsQuizSpace = findViewById(R.id.marsQuizSpace)
        optionOne = findViewById(R.id.optionOne)
        optionTwo = findViewById(R.id.optionTwo)
        quizRight = findViewById(R.id.quizRight)
        quizWrong = findViewById(R.id.quizWrong)
        val marsQuiz = getString(R.string.mars_quiz)
        ...
        fun firstAnim() {
            ...
            Handler().postDelayed({
                marsQuizSpace.visibility = View.VISIBLE
                animateTyping(marsQuizSpace, marsQuiz)
            }, 1000)
    ...
    private fun animateViewsOnIncorrectAnswer() {
        ...
        Handler().postDelayed({
            val process = Intent(this, Mars :: class.java)
            startActivity(process)
            overridePendingTransition(R.anim.fade_in, R.anim.fade_out)
        }, 2000)
    }
    ...
    private fun animateViewsOut() {
        ...
        animation.animateView(marsQuizSpace, Animation.AnimationType.FADE_OUT) {
            marsQuizSpace.visibility = View.INVISIBLE
        }
